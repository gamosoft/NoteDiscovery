...
# Save default config for recovery when bind mount creates a directory
RUN cp config.yaml config.yaml.default

# Entrypoint ensures config.yaml is a file, not a directory
COPY entrypoint.sh .
